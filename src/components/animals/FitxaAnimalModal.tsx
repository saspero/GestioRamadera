...
type Vista = 'general' | 'tractaments'
 ...
 * actual i historial de pesos a la pestanya "General"; historial de
 * tractaments a la pestanya "Tractaments". Botons per aplicar un
 * tractament o donar-lo de baixa al peu.
 *
 ...
 * @returns Modal amb la fitxa de només lectura (amb pestanyes) +
 * ModalBaixa/ModalAplicarTractament opcionals
 *
 * @remarks Pestanyes afegides juliol 2026 (decisió confirmada amb
 * l'usuari) — abans "Historial de medicació" era una secció més
 * dins d'una única vista llarga; ara té la seva pròpia pestanya
 * "Tractaments", amb les mateixes columnes que ja mostrava
 * (Medicament/Data inici/Alliberament) — no s'han afegit camps
 * nous per no assumir res sobre `FitxaAnimal` que no estigués ja
 * confirmat.
 * @remarks Connexió amb ModalAplicarTractament (mode individual,
 * `animalIdPreseleccionat={animalId}`): en confirmar, la invalidació
 * de queryKeys.animals.all que ja fa ModalAplicarTractament també
 * refresca queryKeys.animals.fitxa(animalId) (coincidència de
 * prefix a React Query) — la fitxa es refresca sola.
 * @remarks MIGRACIÓ REACT QUERY: la fitxa es carrega amb useQuery
 * (queryKeys.animals.fitxa(animalId)). ModalBaixa ja invalida
 * queryKeys.animals.all internament en confirmar.
          ...
          <>
            <div className="flex gap-2 border-b border-gray-200">
              <button
                ...
                className={`px-3 py-2 text-sm font-medium border-b-2 -mb-px ${vista === 'general' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              >
                General
              </button>
              <button
                onClick={() => setVista('tractaments')}
                className={`px-3 py-2 text-sm font-medium border-b-2 -mb-px ${vista === 'tractaments' ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
              >
                Tractaments
                {fitxa.historialTractaments.length > 0 && (
                  <span className="ml-1.5 text-xs text-gray-400">({fitxa.historialTractaments.length})</span>
                )}
              </button>
            </div>
            ...
            {vista === 'tractaments' && (
              <div>
                {fitxa.historialTractaments.length === 0 ? (
                  <p className="text-sm text-gray-400 text-center py-8">Cap tractament registrat.</p>
                ) : (
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-gray-500">
                    ...
                    <tbody>
                      {fitxa.historialTractaments.map((t) => (
                        <tr key={t.id} className="border-t border-gray-50">
                          <td className="py-1.5 text-gray-700">{t.nomMedicament}</td>
                          <td className="py-1.5 text-gray-700">{formatDate(t.dataInici)}</td>
