 ...
 *
 * @remarks MIGRACIÓ REACT QUERY/UI: usa el component <Modal> base
 * (src/components/ui/Modal.tsx) per a l'esquelet (overlay, capçalera,
 * tecla Escape). El peu es manté personalitzat amb <div> propi
 * perquè té més estats (idle/completat/reiniciar) del que
 * <ModalAccions> (pensat per a Cancel·lar/Confirmar simple) cobreix.
 * La invalidació de la query d'animals ja la fa useAltaMassiva()
 * internament — aquest component ja no necessita saber-ho.
 * @remarks Control d'accés: es munta des de la pàgina d'animals quan
 * rol === 'Admin'. Els endpoints tornen a validar el rol igualment.
 */
export function ModalAltaMassiva({ onTancar, onImportacioCompletada }: ModalAltaMassivaProps) {
  ...
  return (
    <Modal titol="Alta massiva d'animals" onTancar={onTancar} mida="xl" peu={peu}>
